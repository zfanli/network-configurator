...
快速恢复科学网络配置以及 GitHub SSH & GPG keys。

### 简介

近期（19年初）经历了 windows 购机换机，MacBook 送修等事情，系统格式化了两三次。

由于网络配置完成后不出意外很长一段时间是不再需要重新配置的，所以这方面的记忆不是很完整，这导致每次格式化都伴随 1 ～ 2 天的时间来翻查资料，安装软件，恢复到日常使用状态。

为了节省这这些时间，至少以节省一半时间为目标，整合各种配置 guide，如果可能的话写一个一键配置脚本。
